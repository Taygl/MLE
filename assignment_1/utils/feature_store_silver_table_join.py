...
def feature_store_silver_table(snapshot_date_str, bronze_clickstream_directory, bronze_attribute_directory, bronze_financials_directory, silver_feature_store_directory, spark):
    # prepare arguments
    ...
    print('loaded clickstream_df from:', filepath, 'row count:', clickstream_df.count())

    # connect to bronze table
    partition_name = "bronze_attribute_daily_" + snapshot_date_str.replace('-','_') + '.csv'
    filepath = bronze_attribute_directory + partition_name
    attribute_df = spark.read.csv(filepath, header=True, inferSchema=True)
    print('loaded attribute_df from:', filepath, 'row count:', attribute_df.count())
    ...
    financial_df = spark.read.csv(filepath, header=True, inferSchema=True)
    print('loaded financial_df from:', filepath, 'row count:', financial_df.count())
    ...
    print(snapshot_date_str + 'row count:', features_df.count())
